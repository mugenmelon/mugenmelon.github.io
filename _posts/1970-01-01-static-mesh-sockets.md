...
The short answer: Whenever we need
...
The most straightforward solution would be to go back to Blender and set the center of the sword mesh to the hilt of the sword. And this will work just fine for many projects.
...
Why the *inverse* transform? Because we want the hilt to have the same transform as the parent socket. So we must move the entire mesh in the *opposite* direction.
...
There is much more to it. What I'm trying to say here: A "socket" does not need to look like or be called a "socket". Really all we are talking about are *relative-to-so-*
